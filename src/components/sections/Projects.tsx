...
function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="bg-card-bg border border-border rounded-lg overflow-hidden shadow-sm hover:border-accent/40 transition-all duration-300 flex flex-col group">
      {/* Reduced Image Height (h-28 to h-32) */}
      <div className="relative w-full h-28 sm:h-32 overflow-hidden bg-foreground/5">
      ...
      <div className="p-3 flex flex-col grow justify-between space-y-2">
        <div className="space-y-0.5">
          <h3 className="text-sm font-bold text-foreground group-hover:text-accent transition-colors line-clamp-1">
            {project.title}
          </h3>
          <p className="text-foreground/75 text-[11px] leading-snug line-clamp-2">
            {project.description}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-1 pt-0.5">
          {project.tags.map((tag: string, idx: number) => (
            <span
              key={idx}
              className="text-[9px] px-1.5 py-0.5 rounded bg-accent/10 border border-accent/20 text-accent font-medium leading-none"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border mt-auto">
          <a
            ...
            className="inline-flex items-center gap-1 text-[10px] font-semibold text-foreground/70 hover:text-accent transition-colors"
          >
...
export default function Projects() {
  return (
    <section id="projects" className="py-6 sm:py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden transition-colors duration-300">
      <div className="absolute top-1/3 right-1/4 w-60 h-60 bg-accent/10 blur-3xl rounded-full pointer-events-none -z-10" />
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
          <div className="space-y-1">
            <span className="text-accent text-[10px] font-semibold tracking-[0.2em] uppercase bg-accent/10 px-2 py-0.5 rounded-full border border-accent/20 inline-flex items-center gap-1">
            ...
            <p className="text-foreground/75 text-xs max-w-lg">
          ...
          <Link
            href="/projects"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-card-bg border border-border hover:border-accent hover:text-accent text-foreground font-semibold text-xs transition-all duration-300 group shadow-sm shrink-0 self-start sm:self-auto"
          >
        ...
        {/* Project Cards Grid with max-w-6xl aligned size */}
